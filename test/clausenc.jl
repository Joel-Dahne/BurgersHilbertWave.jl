@testset "_reduce_argument_clausen" begin
    ...
    for x in [xs1; xs2; xs3; xs4; xs5]
        y, haszero, haspi, has2pi = BurgersHilbertWave._reduce_argument_clausen(x)
...
@testset "clausenc" begin
    @testset "clausenc(x, s)" begin
        ...
        for s in [range(Arb(-4), Arb(4), length = 10); Arb.(-3:3)]
            for x in range(Arb(0), 2Arb(π), length = 100)[2:end-1]
                res1 = BurgersHilbertWave._clausenc_polylog(x, s)
                res2 = BurgersHilbertWave._clausenc_zeta(x, s)
                @test isfinite(res1)
                @test isfinite(res2)
        ...
        for s in range(Arb(-4), Arb(4), length = 10)[2:end-1]
            for x in range(Arb(0), 2Arb(π), length = 10)[2:end-1]
                for y in range(Arb(-10), Arb(10), length = 10)
                    ...
                    res2 = BurgersHilbertWave._clausenc_zeta(z, s)
                    @test isfinite(res1)
                    @test isfinite(res2)
                    @test Arblib.overlaps(res1, res2)
                end
        ...
        @test_throws DomainError BurgersHilbertWave._clausenc_zeta(Arb(-1), Arb(2.5))
        @test_throws DomainError BurgersHilbertWave._clausenc_zeta(Arb(7), Arb(2.5))

        # Check evaluation with integer s
        for s in Arb.(-4:4)
            @test Arblib.overlaps(
                BurgersHilbertWave._clausenc_polylog(one(s), s),
                BurgersHilbertWave._clausenc_zeta(one(s), s),
            )
        ...
        for x in range(Arb(0), 2Arb(π), length = 10)[2:end-1]
            for s in Arb.([-1, 1, 2, 3, 4]) # s = 0 not supported
        ...
        for s in [range(Arb(-4), Arb(4), length = 6); Arb.(-3:3)]
            ...
            for x in range(Arb(0), 2Arb(π), length = 20)[2:end-1]
                res1 = BurgersHilbertWave._clausenc_polylog(x, s_series)
                res2 = BurgersHilbertWave._clausenc_zeta(x, s_series)
                @test isfinite(res1)
                @test isfinite(res2)
                @test Arblib.overlaps(res1, res2)
            end
        ...
        for x in range(Arb(0), 2Arb(π), length = 10)[2:end-1]
            for s0 in Arb.([-1, 1, 2, 3, 4]) # s0 = 0 not supported
    ...
    @testset "clausenc(x, s, β)" begin
        ...
        for β in [1, 2]
            for s in [range(Arb(-4), Arb(4), length = 10); Arb.(-3:3)]
                for x in range(Arb(0), 2Arb(π), length = 10)[2:end-1]
                    res1 = BurgersHilbertWave._clausenc_polylog(x, s, β)
                    res2 = BurgersHilbertWave._clausenc_zeta(x, s, β)
                    @test isfinite(res1)
                    @test isfinite(res2)
                    @test Arblib.overlaps(res1, res2)
                end
        ...
        # Check that _clausen_zeta throws an error outside of the domain
        @test_throws DomainError BurgersHilbertWave._clausenc_zeta(Arb(-1), Arb(2.5), 1)
        @test_throws DomainError BurgersHilbertWave._clausenc_zeta(Arb(7), Arb(2.5), 1)
...
@testset "clausenc_expansion" begin
    ...
    for M in [3, 6]
        for x in range(Arb(0), 2Arb(π), length = 100)[1:end-1]
            C, e, P, E = BurgersHilbertWave.clausenc_expansion(x, s, M)
...
@testset "clausencmzeta" begin
    ...
    for s = 2:3
        for x in range(Arb(0), 2Arb(π), length = 100)[2:end-1]
            ...
            res2 = clausenc(x, s, 1) - BurgersHilbertWave.dzeta(Arb(s))
            @test Arblib.overlaps(res1, res2)
        end
    end
end
