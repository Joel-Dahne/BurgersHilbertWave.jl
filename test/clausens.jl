@testset "clausens" begin
    @testset "clausens(x, s)" begin
        ...
        for s in [range(Arb(-4), Arb(4), length = 10); Arb.(-3:3)]
            for x in range(Arb(0), 2Arb(π), length = 100)[2:end-1]
                res1 = BurgersHilbertWave._clausens_polylog(x, s)
                res2 = BurgersHilbertWave._clausens_zeta(x, s)
                @test isfinite(res1)
                @test isfinite(res2)
        ...
        @test_throws DomainError BurgersHilbertWave._clausens_zeta(Arb(-1), Arb(2.5))
        @test_throws DomainError BurgersHilbertWave._clausens_zeta(Arb(7), Arb(2.5))

        # Check evaluation with integer s
        for s in Arb.(-4:4)
            @test Arblib.overlaps(
                BurgersHilbertWave._clausens_polylog(one(s), s),
                BurgersHilbertWave._clausens_zeta(one(s), s),
            )
        ...
        for s in [range(Arb(-4), Arb(4), length = 6); Arb.(-3:3)]
            ...
            for x in range(Arb(0), 2Arb(π), length = 20)[2:end-1]
                res1 = BurgersHilbertWave._clausens_polylog(x, s_series)
                res2 = BurgersHilbertWave._clausens_zeta(x, s_series)
                @test isfinite(res1)
                @test isfinite(res2)
                @test Arblib.overlaps(res1, res2)
            end
    ...
    @testset "clausens(x, s, β)" begin
        ...
        for β in [1, 2]
            for s in [range(Arb(-4), Arb(4), length = 10); Arb.(-3:3)]
                for x in range(Arb(0), 2Arb(π), length = 10)[2:end-1]
                    res1 = BurgersHilbertWave._clausens_polylog(x, s, β)
                    res2 = BurgersHilbertWave._clausens_zeta(x, s, β)
                    @test isfinite(res1)
                    @test isfinite(res2)
                    @test Arblib.overlaps(res1, res2)
                end
        ...
        # Check that _clausen_zeta throws an error outside of the domain
        @test_throws DomainError BurgersHilbertWave._clausens_zeta(Arb(-1), Arb(2.5), 1)
        @test_throws DomainError BurgersHilbertWave._clausens_zeta(Arb(7), Arb(2.5), 1)
...
@testset "clausens_expansion" begin
    ...
    for M in [3, 6]
        for x in range(Arb(0), 2Arb(π), length = 100)[1:end-1]
            C, e, P, E = BurgersHilbertWave.clausens_expansion(x, s, M)
