...
The bound for this is based on [`lemma_U1_asymptotic`](@ref), though
we also include most of the details below.
...
integrand is based on [`lemma_I_hat`](@ref).
...
For the second term we use [`lemma_U11_bound_zero`](@ref), which says
that the whole term is decreasing for `t < inv(10x)`.
"""
...
[`lemma_U_singular_integrals`](@ref), though we also include most of
the details below.
